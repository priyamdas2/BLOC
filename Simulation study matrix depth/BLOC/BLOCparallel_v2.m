function [C_opt, fval, comp_time] = BLOCparallel_v2(objFun, C0, varargin)
...
for iii = 1:MaxRuns
    ...
    for i = 1:MaxIter
        ...
        total_lh = current_lh * ones(2*N,1);
        
        parfor location_number = 1:(2*N)
            ...
            if change_loc == 1
                ...
                elseif ismember(change_loc, start_element_locs)
        ...
        if candidate < current_lh
            ...
            % Reconstruct ONLY winning candidate
            change_loc = ceil(I/2);
            epsilon_temp = ((-1)^I)*epsilon;
            
            theta_best = theta;
            possibility_temp0 = theta(change_loc) + epsilon_temp;
            ...
            theta_best(change_loc) = possibility_temp;
            theta = theta_best;
        end
        
        array_of_values(i) = candidate;
...
fprintf('xxxxxxxxxxxxxxxxxxxxxx parallel-BLOC ends xxxxxxxxxxxxxxxxxxxxxxxxxx\n')
