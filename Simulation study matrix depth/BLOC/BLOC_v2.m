function [C_opt, fval, comp_time] = BLOC_v2(objFun, C0, varargin)
    ...
    for i = 1:MaxIter
        ...
        for location_number = 1:(2*N)
            ...
            if all(diag(possibility_L) > 1e-3)
                ...
                possibility_lh = objFun(possibility_Corr);
                
                % Store ONLY the current best candidate
                if possibility_lh < candidate
                    candidate = possibility_lh;
                    theta_best = possibility;
                end
            end
        end
        
        % Accept best candidate evaluated before timeout
