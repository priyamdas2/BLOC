function ll_MCP = log_likelihood_gaussian_MCplus(X, Sigma, lambda, gamma, P)
% Computes penalized log-likelihood of multivariate Gaussian data X
% using covariance matrix Sigma, with MC+ penalty weighted by mask matrix P.
%
% Inputs:
%   X      : n x d data matrix (assumed zero-mean)
%   Sigma  : d x d positive definite covariance matrix
%   lambda : scalar penalty coefficient
%   gamma  : scalar > 1, controls the shape of the MC+ penalty
%   P      : d x d binary mask (1 = penalize, 0 = ignore)
    ...
    mcplus_penalty = sum(arrayfun(@(t) MCplus_scalar(t, lambda, gamma), abs_vals));

    % Penalized log-likelihood
    ll_MCP = ll - mcplus_penalty;
end

function val = MCplus_scalar(theta, lambda, gamma)
% MC+ penalty for a single scalar theta
    if abs(theta) <= gamma * lambda
        val = lambda * abs(theta) - (theta^2) / (2 * gamma);
    else
        val = 0.5 * gamma * lambda^2;
    end
end
