function ll = log_likelihood_gaussian_LASSO(X, Sigma, lambda, P)
% Computes penalized log-likelihood of multivariate Gaussian data X
% using covariance matrix Sigma, with L1 penalty weighted by mask matrix P.
%
% Inputs:
...
%   P      : d x d binary mask (1 = penalize, 0 = ignore)
    ...
    % Custom L1 penalty
    l1_penalty = lambda * sum(abs(Sigma(P == 1)));
